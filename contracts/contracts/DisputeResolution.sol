...
contract DisputeResolution is ReentrancyGuard, Ownable {
    ...
    uint256 public constant NUM_JURORS = 3; // 3 jurors
    ...
    struct Dispute {
        ...
        address[3] selectedJurors; // Top 3 jurors (set bởi server)
        bytes32[3] commitHashes; // Hash của phiếu bầu
        uint8[3] revealedVotes; // 0=chưa/abstain, 1=buyer thắng, 2=seller thắng
        bool[3] hasCommitted;
        bool[3] hasRevealed;
        DisputePhase phase;
    ...
    event JurorsAssigned(uint256 indexed disputeId, address[3] jurors);
    ...
    function stakeForJuror(uint256 amount) external nonReentrant {
        ...
        require(
            adfToken.transferFrom(msg.sender, address(this), amount),
            "Stake failed"
        );
    ...
    /// @notice Chỉ định danh sách 3 ví trọng tài (Jurors)
    /// @dev Chỉ cho phép serverOracle gọi
    function setJurors(
        uint256 _disputeId,
        address[3] calldata _jurors
    ) external {
        ...
        for (uint8 i = 0; i < NUM_JURORS; i++) {
            require(
                _jurors[i] != dispute.buyer && _jurors[i] != dispute.seller,
                "Juror conflict"
            );
            require(
                jurorStakes[_jurors[i]] >= MIN_JUROR_STAKE,
    ...
    function revealVote(
        ...
        require(
            calculatedHash == dispute.commitHashes[jurorIndex],
            "Hash mismatch"
        );
    ...
    function resolveDispute(uint256 _disputeId) external nonReentrant {
        ...
        for (uint8 i = 0; i < NUM_JURORS; i++) {
            ...
            bool voteCorrectly = (buyerWins && dispute.revealedVotes[i] == 1) ||
            ...
            if (voteCorrectly) {
                ...
                uint256 penalty = JUROR_PENALTY > jurorStakes[juror]
                    ? jurorStakes[juror]
                    : JUROR_PENALTY;
                ...
                require(
                    adfToken.transfer(address(adfPool), penalty),
        ...
        if (
            totalReward > 0 &&
            address(adfPool) != address(0) &&
            adfPool.reserveADF() >= totalReward
        ) {
            adfPool.withdrawJurorReward(totalReward);
            for (uint8 i = 0; i < NUM_JURORS; i++) {
                if (dispute.hasRevealed[i]) {
                    bool correct = (buyerWins &&
                        dispute.revealedVotes[i] == 1) ||
                        (!buyerWins && dispute.revealedVotes[i] == 2);
                    if (correct) {
    ...
    function triggerGameTheoryBurn(uint256 _disputeId) external {
        ...
        emit DisputeResolved(_disputeId, address(0), 0, 0, 0);
    }

    // Helper kiểm tra ví trong danh sách trọng tài
    function getJurorIndex(
        uint256 _disputeId,
        address _juror
